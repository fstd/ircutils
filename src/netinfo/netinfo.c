...
bool conread_cb(char *(*tok)[MAX_IRCARGS], void *tag);
...
main(int argc, char **argv)
{
	...
	for (;;) {
		...
		char line[1024];
		sndumpmsg(line, sizeof line, 0, &tok);
		D("LO: %s", line);
...
bool
conread_cb(char *(*tok)[MAX_IRCARGS], void *tag)
{
	char line[1024];
	sndumpmsg(line, sizeof line, tag, tok);
	D("CR: %s", line);
